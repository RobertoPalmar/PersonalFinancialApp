...
class GetGeneralDashboardDataUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): DashboardData? {
        try {
            ...
            val totalBalance =
                accountList.sumOf { calculateExchangeRate(it.account.balance,it.currency.currentExchangeRate) }

            // CALCULATE INCOME
            val incomeBalance = currentMonthTransactionList
                .filter { it.transaction.transactionType == TransactionType.INCOME }
                .sumOf { calculateExchangeRate(it.transaction.amount, it.currency.currentExchangeRate) }
